...
contract FoundingEvent {
	...
	function _createLiquidity() internal {
		...
		if (tknFTMLP == address(0)) {
			tknFTMLP=I(factory).createPair(_letToken, WFTM);
		}
		...
		I(router).addLiquidityETH{value: address(this).balance}(_letToken,amount,0,0,staking,2**256-1);
		I(staking).genesis(balance, tknFTMLP,block.number);
		I(_letToken).transfer(treasury,I(_letToken).balanceOf(address(this)));// burn excess to treasury, in case if hardcap is not reached by the end date is
		I(_letToken).genesis(block.number);
		I(_treasury).genesis(block.number);
		delete _lgeOngoing;
	}
